...
DGM'nin `is_compiled_self_improve()` (`utils/evo_utils.py:96-126`) kod kalitesi/güvenlik kontrolü içermiyor — yazarın kendi itirafı (`README.md:88`) bunu doğruluyor. OpenEvolve'da da `evaluator.py` keyfi kodu izolasyonsuz (yalnızca `multiprocessing`) çalıştırıyor; repo genelinde "reward hack/exploit" uyarısı bulunamadı (kendini-degistiren-sistemler.md).
