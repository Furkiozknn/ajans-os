...
- **Bütçe (budget_limiter):** `provider_spend:{provider}:{duration}`, `deployment_spend:{id}:{duration}`, `tag_spend:{tag}:{duration}` anahtarları (`litellm/router_strategy/budget_limiter.py:249`, `:261-262`; `provider_spend` `:370`), TTL = bütçe penceresinin saniye karşılığı — `budget_limiter.py:341-375` (`_get_or_set_budget_start_time`). *(Adresler denetimde düzeltildi.)*
