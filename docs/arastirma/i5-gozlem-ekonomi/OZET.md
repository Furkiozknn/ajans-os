...
- `langfuse.md:119` — `usageDetails`/`costDetails` açık uçlu sözlük, ama `OtelIngestionProcessor.ts:2846-2881` OpenAI ve Vercel AI SDK için ayrı if/else blokları.
...
- `nvidia-nim-mcp.md:118-121` — NVIDIA modelleri `api_base` sarmalaması istiyor, diğer sağlayıcılar istemiyor; ayrıca `VISION_PROVIDERS` (`:168-172`) ikinci bir liste zorunlu kılınmış.
...
2. **Kullanılabilir ekonomi verisi toplanıp atılıyor.** `nvidia-nim-mcp.md:148` — `litellm.acompletion` yanıtı `usage` alanı taşıyor ama kod yalnızca `content`/`model` okuyor, token sayısı bedavaya elde iken atılıyor. `model-comparison-harness.md` — yargıç çağrısının kendi maliyeti hiç ölçülmüyor, oysa `--rubric` her satırda ek bir LLM çağrısı demek. İki proje bağımsız olarak "ölçülebilecek ekonomi sinyalini ölçmeme" hatasına düşmüş.
